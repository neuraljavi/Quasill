...
def encode(text, tokenizer, vocab, max_length):
    # Tokeniza el texto
    totenized_text = tokenizer(text)

    # Codifica el texto
    encoded_text = [vocab.stoi[token] for token in totenized_text]

    # Añade padding
    if len(encoded_text) < max_length:
        encoded_text += [vocab.stoi['<pad>']] * (max_length - len(encoded_text))
    else:
...
def get_feedback(text, label):
    ...
    loss = update_model(classifier, text, label, tokenizer, vocab)
    return loss
